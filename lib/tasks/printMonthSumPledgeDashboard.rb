    ...
    rowBuffer += '
      {
        ...
        targets: [
          {
            \'target\': "alias(scale(summarize(sumSeries(scale(faust_pledge.by_site.' + site.name + '.by_vo.*.specInt2k,0.00274)), \'$binning\', \'sum\', true),0.4167),\'pledge Ksi2k\')"
          },
          {
            \'target\': "alias(summarize(scale(summarize(sumSeries(faust.cpu_grid_norm_records.by_site.' + site.name + '.by_vo.$vo.wall_H_KSi2k),\'1d\',\'avg\'),0.04167),\'$binning\',\'sum\'),\'[Ksi2k][days] - all\')"
          },
          {
