class AlterCloudRecordSummaries < ActiveRecord::Migration
  def up
    ...
    select 
        ...
        cast(`cloud_view_vm_summaries`.`date`
            as date) AS `date`,
        hour(`cloud_view_vm_summaries`.`date`) AS `hour`,
    ...
    group by date(`cloud_view_vm_summaries`.`date`) , hour(`cloud_view_vm_summaries`.`date`) , `resources`.`site_id` , `cloud_view_vm_summaries`.`status` , `cloud_view_vm_summaries`.`local_group` , `cloud_view_vm_summaries`.`local_user`')
  end
  def down
