class AlterCloudViewVmSummaries < ActiveRecord::Migration
  def up
    
    CloudViewVmSummary.connection.execute('
      CREATE OR REPLACE VIEW
    `cloud_view_vm_summaries` AS
    select 
        `cloud_records`.`id` AS `id`,
        `cloud_records`.`endTime` AS `date`,
    ...
    from
        `cloud_records`
    group by `cloud_records`.`endTime` , `cloud_records`.`VMUUID` , `cloud_records`.`status`')
  end
  def down
